...
from mlmc.random import fracture as frac
from collections import defaultdict
# TODO:
# - debug continuous position generation
# - fix area for aspect
# - set fracture unit shape
...
def test_power_law(volume, intensity, size_range, kappa):
    ...
    p_law.set_lower_bound_by_intensity(intensity / 2)
...
def test_intensity_p_32():
    ...
    pop.set_sample_range([1, 200], sample_size=fr_size)
...
def test_ConnectedPosition():
    volume = 600 ** 3
    pop = frac.Population(volume)
    fr_ori = frac.FisherOrientation(trend=45, plunge=45, concentration=1)
