...
class SamplingPoolPBS(SamplingPool):
    ...
    def _create_job_dir(self):
        """
        Create job directory - necessary files there for each tasks
        :return: None
        """
    ...
    def pbs_common_setting(self, **kwargs):
        ...
        self._pbs_header_template.extend((
                                         '{python} {pbs_process_file_dir}/pbs_process.py {output_dir} {job_name} >'
    ...
    def execute(self):
        ...
        if len(self._scheduled) > 0:
            ...
            self.write_pbs_job_file(script_content, job_file)
            self._job_count += 1
            ...
            # process = subprocess.run(['qsub', os.path.join(self._jobs_dir, PbsProcess.JOB.format(job_id))], stderr=subprocess.PIPE, stdout=subprocess.PIPE)
            # if process.returncode != 0:
            #     raise Exception(process.stderr.decode('ascii'))
            # self._pbs_id_from_qstat(process)
    ...
    def write_pbs_job_file(self, content, job_file):
        with open(job_file, "w") as f:
            f.write(content)
        os.chmod(job_file, 0o774)

    def _pbs_id_from_qstat(self, output):
        """
    ...
    def get_finished(self):
        ...
        finished_pbs_jobs = self._pbs_ids
        unfinished_pbs_jobs = []
    ...
    def get_result_files(self, finished_pbs_jobs, unfinished_pbs_jobs):
        ...
        for pbs_id in finished_pbs_jobs:
            ...
            if file:
                ...
                # Get sample results
                successful, failed, time = self._pbs_process.read_results(job_id)
