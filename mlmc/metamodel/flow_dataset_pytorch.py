...
class FlowDataset(Dataset):
    ...
    def get(self, index):
        print("GET self.dataset.loc[index] ", self.dataset.loc[index])
    ...
    def _deep_map_preprocessing(self):
        import mlmc.metamodel.DeepMap.graph_canonicalization as gc
        from sklearn.model_selection import train_test_split, StratifiedKFold, KFold
        from keras.layers import Input, Flatten, Dense, Dropout, Conv1D, \
            Lambda, GlobalMaxPooling1D, MaxPooling1D, GlobalAveragePooling1D, BatchNormalization, Activation, \
            Concatenate
        ...
        start = time.time()
        X, feature_size, num_sample, graph_labels = gc.canonicalization(self.a, self.data, filter_size, feature_type,
                                                          graphlet_size, max_h)


        labels = list(graph_labels.values())
        print("labels ", labels)
        print("num sample", num_sample)
        folds = list(KFold(n_splits=kfolds, shuffle=True, random_state=7).split(np.zeros(len(labels)),
                                                                                          labels))
        ...
        for j, (train_idx, test_idx) in enumerate(folds):
            print('\nFold ', j)
            scores_val_acc, scores_acc = FlowDataset.train_model(X, y, train_idx, test_idx, num_sample, feature_size,
                                                                 num_class, batch_size, EPOCHS, filter_size)
            # print(scores)
    ...
    def get_model(filter_size, num_instance, feature_size, num_class):
        ...
        print("Input x ", x)
        flow = Conv1D(filters=32, kernel_size=filter_size, strides=filter_size, padding='same')(x)
        flow = Activation('relu')(flow)

        flow = Conv1D(filters=16, kernel_size=3, strides=1, padding='same')(flow)
        flow = Activation('relu')(flow)

        flow = Conv1D(filters=8, kernel_size=1, strides=1, padding='same')(flow)
        flow = Activation('relu')(flow)
        ...
        predictions = Dense(1, activation='linear')(flow)
    ...
    def train_model(X, y, train_idx, test_idx, num_sample, feature_size, num_class, batch_size, EPOCHS, filter_size):
        ...
        y_val = np.array([y[id] for id in test_idx])

        X_train = [X[id].toarray() for id in train_idx]

        print("X_train ", X_train[0].shape)

        y_train = np.array([y[id] for id in train_idx])
        print("y_train ", y_train)

        callbacks = FlowDataset.get_callbacks(patience_lr=5)

        model = FlowDataset.get_model(filter_size, num_sample, feature_size, num_class)
        model.summary()

        result = model.fit(x=np.array(X_train), y=y_train, batch_size=batch_size,
