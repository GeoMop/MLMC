...
from mlmc.metamodel.own_src import SRCPool
...
class DiffPool(SRCPool):
    ...
    **Arguments**
    ...
    def select(self, x, a, i, fltr=None, mask=None):
        print("select x ",x )
        print("kernel pool ", self.kernel_pool)
        print("fltr ", fltr)
        print("mask ", mask)

        s = ops.modal_dot(fltr, K.dot(x, self.kernel_pool))
        print("s before softmax ", s)
        s = activations.softmax(s, axis=-1)
        print("s from softmax ", s)
        #s = activations.relu(s, axis=-1)
        if mask is not None:
            s *= mask[0]
